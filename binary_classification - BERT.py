...
class BERTClassificationCollator(object):
    """
    Data Collator used for BERT in a classificaiton rask. 
    
    It uses a given tokenizer and label encoder to convert any text and labels to numbers that 
    can go straight into a BERT model.
...
model_config = BERTConfig.from_pretrained(pretrained_model_name_or_path=BERT_PRETRAINED_CONFIG_ARCHIVE_MAP['bert'], num_labels=2)
...
BERT_classificaiton_collator = BERTClassificationCollator(use_tokenizer=tokenizer, max_sequence_len=max_length)


print('Dealing with Train...')
# Create pytorch dataset.
BERT_dataset = MalBengDataset(path='data_1', use_tokenizer=tokenizer)
print('Created `all_dataset` with %d examples!'%len(BERT_dataset))


#X_train, X_test, y_train, y_test = train_test_split(df, y, test_size=0.2)

total_count=len(BERT_dataset) #splt data for train,validation and train
...
train_dataset, valid_dataset= torch.utils.data.random_split(
    BERT_dataset, (train_count, valid_count))#+1 ekledim neden?anlamadım
